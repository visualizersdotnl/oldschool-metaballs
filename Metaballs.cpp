...
bool Metaballs::Initialize()
{
	...
	delete[] signature; // FIXME: potential leak on throw.

	// Get shader constant indices.
	varIndexEnvMap = effect.RegisterVariable("envMap", true);
	varIndexProjMap = effect.RegisterVariable("projMap", true);
	varIndexViewProjMatrix = effect.RegisterVariable("mViewProj", true);
...
void Metaballs::Generate(unsigned int numBall4s, const Metaball4 &balls, float surfaceLevel)
{
	...
	for (unsigned int iBall4 = 0; iBall4 < numBall4s; ++iBall4)
	{
		for (unsigned int iBall = 0; iBall < 4; ++iBall)
		{
			...
			float gridX = (s_pBall4s[iBall4].X[iBall]*0.5f + 0.5f) * kGridCubes;
			float gridY = (s_pBall4s[iBall4].Y[iBall]*0.5f + 0.5f) * kGridCubes;
			float gridZ = (s_pBall4s[iBall4].Z[iBall]*0.5f + 0.5f) * kGridCubes;
...
void Metaballs::Draw(Camera* camera)
{
	...
	gD3D->BindIndexBuffer32(pIB);

	// Set shader vars.
	effect.SetVariableValue(varIndexEnvMap, envMap->GetShaderResourceView());
...
void Metaballs::SetMaps(Texture2D *envMap, Texture2D *projMap, float projScrollU /* = 0.f */, float projScrollV /* = 0.f */)
{
	ASSERT(nullptr != envMap);
	this->envMap = envMap;
	this->projMap = (nullptr != projMap) ? projMap : gD3D->GetWhiteTex();
...
void Metaballs::ProcessCube(unsigned int iGrid, unsigned int iX, unsigned int iY, unsigned int iZ)
{
	...
	if ((s_gridCache[iGrid] & 0xff000000) == 0xff000000)
	{
		...
		const float gridX = -1.f + iX*kGridStep;
		const float gridY = -1.f + iY*kGridStep;
		const float gridZ = -1.f + iZ*kGridStep;
